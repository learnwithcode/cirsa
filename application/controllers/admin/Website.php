...
class Website extends MY_Controller {
	...
	public function __construct(){
	  // Call the Model constructor
	   parent::__construct();
		...
		if($form_data['submitsms1']==1){
		    $apiid = FCrtRplc($form_data['apiid']);
		    $user = FCrtRplc($form_data['user']);
			$authkey = FCrtRplc($form_data['authkey']);
			$senderid = FCrtRplc($form_data['senderid']);
			$url = FCrtRplc($form_data['url']);
			...
			$this->SqlModel->updateRecord(prefix."tbl_set_api",$data,array("id"=>$apiid));
				$this->SqlModel->updateRecord(prefix."tbl_set_api",array("status"=>'N'),array("id"=>2));
				$this->SqlModel->updateRecord(prefix."tbl_set_api",array("status"=>'N'),array("id"=>3));
			//PrintR($form_data);
			
			
			set_message("success","Successfully updated changes");
			redirect_page("website","sms_api",array());
		}
		
			if($form_data['submitsms2']==1 ){
	       $apiid = FCrtRplc($form_data['apiid']);
		    $user = FCrtRplc($form_data['user']);
			$authkey = FCrtRplc($form_data['authkey']);
			$senderid = FCrtRplc($form_data['senderid']);
			...
			$this->SqlModel->updateRecord(prefix."tbl_set_api",$data,array("id"=>$apiid));
					$this->SqlModel->updateRecord(prefix."tbl_set_api",array("status"=>'N'),array("id"=>1));
				$this->SqlModel->updateRecord(prefix."tbl_set_api",array("status"=>'N'),array("id"=>3));
		//	PrintR($form_data);
			set_message("success","Successfully updated changes");
			redirect_page("website","sms_api",array());
		...
		$this->load->view(ADMIN_FOLDER.'/website/sms_api',$data);
	}
	public function popupwebstatus()
	{
	$status = $this->input->post('status');
	$popupId = $this->input->post('popupId');
	if($status =='false')
	{
	$data = array('web_status'=>'N');
	$this->SqlModel->updateRecord(prefix."tbl_popup_data",$data,array("popup_id"=>$popupId));
	echo "0";
	}
	elseif($status =='true')
	{
    $data = array('web_status'=>'N');
	$this->SqlModel->updateRecord(prefix."tbl_popup_data",$data);
	$data = array('web_status'=>'Y');
	$this->SqlModel->updateRecord(prefix."tbl_popup_data",$data,array("popup_id"=>$popupId));
	echo "1";
	}
	else
	...
	public function popupsoftstatus()
	{
	$status = $this->input->post('status');
	$popupId = $this->input->post('popupId');
	if($status =='false')
	{
	$data = array('soft_status'=>'N');
	$this->SqlModel->updateRecord(prefix."tbl_popup_data",$data,array("popup_id"=>$popupId));
	echo "0";
	}
	elseif($status =='true')
	...
	{
	$data = array('status'=>'0');
	$this->SqlModel->updateRecord(prefix."tbl_banner",$data,array("banner_id"=>$bannerId));
	...
	$oprt_id  = $this->session->userdata('oprt_id');
		...
		$this->load->view(ADMIN_FOLDER.'/website/banner',$data);
	}
...
$action_request = ($form_data['action_request'])? $form_data['action_request']:$segment['action_request'];
		$banner_id = ($form_data['bannerId'])? $form_data['bannerId']:$segment['banner_id'];
		switch($action_request){
			case "ADD_UPDATE":
		if($form_data['submit_data']==1 && $this->input->post()!='')
		{
			
		if($form_data['action_request']=='ADD_UPDATE')
		{
		
		  if($form_data['bannerId'] > 0)
		  {
		  $banId = $form_data['bannerId'];
		  $AR_BAN = SelectTable("tbl_banner","image","banner_id='$banId'");
		  $photo =$AR_BAN['image']; 
		 ...
		 $ext = explode(".",$_FILES['banner']["name"]);
				...
				$target_path = $fldvPath."upload/banner/".$photo;	
			   
				$final_location = $fldvPath."upload/banner/".$AR_BAN['image'];
		 ...
		 $posted_data = array('banner_title' => $form_data['title'],'url'=>$form_data['url'],'image'=>$photo );
		$this->SqlModel->updateRecord(prefix."tbl_banner",$posted_data,array("banner_id"=>$banId));
		 set_message("success","Successfully updated Banner .");
				    redirect_page("website","banner",array()); 
		  }
		  else
		  {
		  ...
		  $posted_data = array('banner_title' => $form_data['title'],'url'=>$form_data['url'],'date_time'=>date('Y-m-d H:i:s') );
		  $banner_id = $this->SqlModel->insertRecord(prefix."tbl_banner",$posted_data);
	...
	$this->SqlModel->updateRecord(prefix."tbl_banner",array("image"=>$photo),array("banner_id"=>$banner_id));
					
						
					}
		  }
					set_message("success","Successfully added new Banner .");
				    redirect_page("website","banner",array()); 
			}
			else
			{
			set_message("warning","Please Select a Image File.");
			redirect_page("website","banneradd",array()); 
			}
		...
		break;
			...
			break;
			case "EDIT":
				$QR_PAGE ="SELECT * FROM ".prefix."tbl_banner WHERE banner_id='$banner_id'";
		...
		$this->load->view(ADMIN_FOLDER.'/website/banneradd',$data);
	}
	...
	$oprt_id  = $this->session->userdata('oprt_id');
		...
		$fetchRow = $sel_query->row_array();
		$data['fetchRow']=$fetchRow;
		$this->load->view(ADMIN_FOLDER.'/website/popuplist',$data);
	}
	...
	$model = new OperationModel();
		...
		$form_data = $this->input->post();
				$segment = $this->uri->uri_to_assoc(2);
    $action_request = ($form_data['action_request'])? $form_data['action_request']:$segment['action_request'];
	$popup_id = ($form_data['popup_id'])? $form_data['popup_id']:$segment['popup_id'];
		switch($action_request){
			case "ADD_UPDATE":
		if($form_data['submit_data']==1 && $this->input->post()!='')
		{
		  $soft_status =$form_data['status'];	
		if($form_data['action_request']=='ADD_UPDATE')
		{
		
		  if($form_data['popup_id'] > 0)
		  {
		  $popup_id = $form_data['popup_id'];
		  
		  $AR_BAN = SelectTable("tbl_popup_data","file","popup_id='$popup_id'");
		  $photo  = $AR_BAN['file']; 
		 if($_FILES['popup']['error']=="0"){
		 
		 $ext = explode(".",$_FILES['popup']["name"]);
				$fExtn = strtolower(end($ext));
					if($fExtn=='png' or $fExtn=='gif' or $fExtn=='jpeg' or $fExtn=='jpg'){
				$fldvUniqueNo = UniqueId("UNIQUE_NO");
				$photo = $fldvUniqueNo.rand(100,999)."_". str_replace(" ","",rand(100,999).".".$fExtn);
				$target_path = $fldvPath."upload/popup/".$photo;	
			   
				$final_location = $fldvPath."upload/popup/".$AR_BAN['file'];
				$fldvImageArr= @getimagesize($final_location);
				if ($fldvImageArr['mime']!="") { @chmod($final_location,0777);	@unlink($final_location); }		
				move_uploaded_file($_FILES['popup']['tmp_name'], $target_path);
				
				
		 }
				
		 }
		 $posted_data = array('file'=>$photo,'soft_status'=>$soft_status );
		$this->SqlModel->updateRecord(prefix."tbl_popup_data",$posted_data,array("popup_id"=>$popup_id));
		 set_message("success","Successfully updated Popup .");
				    redirect_page("website","popuplist",array()); 
		  }
		  else
		  {
		  ...
		  if($_FILES['popup']['error']=="0"){
				...
				$target_path = $fldvPath."upload/popup/".$photo;
					...
					if(move_uploaded_file($_FILES['popup']['tmp_name'], $target_path)){
				 $posted_data = array('file' => $photo,'soft_status'=>$soft_status ,'added_date_time'=>date('Y-m-d H:i:s') );
		         $this->SqlModel->insertRecord(prefix."tbl_popup_data",$posted_data);
			...
			{
			set_message("warning","Please Select a Image File.");
			redirect_page("website","popupadd",array()); 
			}
		...
		break;
			case "DELETE":
				...
				$final_location = $fldvPath."upload/popup/".$photo;
				$fldvImageArr= @getimagesize($final_location);
				if ($fldvImageArr['mime']!="") { @chmod($final_location,0777);	@unlink($final_location); }		
		  
					$model->deleteTable(prefix."tbl_popup_data",array("popup_id"=>$popup_id));
					set_message("success","You have successfully deleted Popup");}else{
					set_message("warning","Failed , unable to delete Popup");
				}
				redirect_page("website","popuplist",array()); exit;
			break;
			case "EDIT":
				$QR_PAGE ="SELECT * FROM ".prefix."tbl_popup_data WHERE popup_id='$popup_id'";
				$SEL_QUERY = $this->db->query($QR_PAGE);
				$AR_PAGE = $SEL_QUERY->row_array();
				$data['ROW'] = $AR_PAGE;
			break;
		...
		$this->load->view(ADMIN_FOLDER.'/website/popupadd',$data);
	}
