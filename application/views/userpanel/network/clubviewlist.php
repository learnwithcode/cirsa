...
//	PrintR(_d($segment['id']));
	
	$clubmember_ID =_d($segment['id']);
	...
	$userid  = $model->getMemberUserId($clubmember_ID);
...
// 		 ORDER BY tm.team_bv DESC";
		 ...
		 $QR_PAGE   = $this->SqlModel->runQuery("SELECT user_id, count_directs ,member_id,club_id, SUM(self_bv) as self_bv , SUM(team_bv) as team_bv FROM `tbl_members` WHERE sponsor_id =  '$clubmember_ID'  and subcription_id > 0 GROUP BY member_id ORDER BY `team_bv` DESC  ");   
...
<h1 class="page-header">
  Team Business of <?php echo $userid; ?> </h1>
<p> <?php 
 $member_idddd = $model->getMemberId($userid);


$memberclubrankde = $model->getclubrank($member_idddd);
...
<table class="table mb-0">
     <thead>
				...
				foreach($QR_PAGE as $AR_DT):  
				 ...
				 $clubrankde = $AR_DT['club_id'];//$model->getclubrank($AR_DT['member_id']);
