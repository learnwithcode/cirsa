...
// 		 ORDER BY tm.team_bv DESC";
		 ...
		 $QR_PAGE   = $this->SqlModel->runQuery("SELECT user_id, count_directs , SUM(self_bv) as total, SUM(team_bv) as total1, SUM(r_self_bv+r_team_bv) as total2  FROM `tbl_members` WHERE sponsor_id =  '$member_id'  and subcription_id > 0 GROUP BY member_id ORDER BY `total` DESC  ");   
              ...
              <tr   >
                ...
                <td><?php echo CURRENCY; ?> <?php echo number_format($AR_DT['total'],2); ?></td>
                          <td><?php echo CURRENCY; ?> <?php echo number_format($AR_DT['total1'],2); ?></td>
                             <td><?php echo CURRENCY; ?> <?php echo number_format($AR_DT['total2'],2); ?></td>
