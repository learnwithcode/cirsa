<?php defined('BASEPATH') OR exit('No direct script access allowed'); 
	...
	$account = new Account();
	$this->OperationModel->deleteOldCoinBase($member_id,$trns_type);
	...
	if($AR_COIN['coinbase_id']=='' || $AR_COIN['coinbase_id']=='0' || !isset($AR_COIN['coinbase_id'])){	
		...
		$this->OperationModel->setBitcoinProcessUrl($coinbase_id,$type_id,$trns_type,"upgrademember");
    ...
    <script>	
		...
		$(function(){
			...
			function check_payment_status(){
				...
				var URL_LOAD= "<?php echo BASE_PATH; ?>json/jsonhandler?switch_type=BITCOIN_PAYMENT_UPGRADE";
				//alert(URL_LOAD);
				var data = {
				...
				$.getJSON(URL_LOAD,data,function(jsonVal){
					if(jsonVal.error_msg!=''){
						switch(jsonVal.error_msg){
							...
							break;
							case "partly":
								$("#paymentLoader").html('<i class="fa fa-spinner fa-spin animated"></i>'+jsonVal.error_dtl+'');
								$("#payment_sts_button").html('Check your payment status');
								$("#button_div").html('<a target="_blank" href="'+jsonVal.link_check+'"> Check your payment status </a>');
							break;
							default:
