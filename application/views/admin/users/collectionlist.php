...
if($segment['type'] =='NEW_COLLECTION')
{
 $processId =$processId +1;
}

$QR_PAGES="SELECT tm.member_id,sub.date_from as active_date , pin.mrp,pin.prod_pv,tm.user_id,tm.date_join,CONCAT_WS(' ',tm.first_name,tm.midle_name,tm.last_name) as name,sm.user_id as suserId,tm.city_name FROM `tbl_process` as prs LEFT JOIN tbl_subscription as sub on sub.date_from BETWEEN prs.start_date and prs.end_date LEFT JOIN tbl_pintype as pin on sub.type_id = pin.type_id 
LEFT JOIN tbl_members as tm on tm.member_id = sub.member_id 
LEFT JOIN tbl_members as sm on sm.member_id = tm.sponsor_id 
WHERE prs.process_id='$processId' /*and sub.active_by='0'*/";
                  ...
                  <div class="table-responsive project-list">
			       ...
			       ?>
                        <tr>
                          ...
                          <td><?php if($AR_DT['mrp'] > 0 ) { echo $AR_DT['mrp']; } else{ echo $model->getprodpack($AR_DT['member_id']);}?>[<?php echo $AR_DT['prod_pv']; ?>Pv ]</td>
